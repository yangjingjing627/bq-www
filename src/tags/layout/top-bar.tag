<top-bar>
		...
		<div class="product-list">
			...
			<ul class="top-goods-list">
				<li each={ item in goodList } onclick="{ addToCart(item) }">
					<div class="item-pic" style={ item.imageUrl ? 'background-image:url('+ item.imageUrl +')' : 'background-image:url(imgs/default-140.png);background-size:70px'}></div>
	...
	<script>
		...
		self.toggleProducts = function () {
			...
			self.update();
			// 这一段的作用是不用每次点击都重新请求，而且保持滚动条位置－－现因为初始化加载有时加载数据不全改为每次点击＋都请求
			// 修改为 start
		...
		self.initCategory = function () {
			store.category.get(function(data){
				...
				$(".product-list").css("height", self.cateHeight);
					// self.cateHeight = $(".top-category").height();
					// $("#product-layer").css("padding-top", self.cateHeight);
