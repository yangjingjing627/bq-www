	...
	<div class="half re-right">
		<div class="card order-detail-wraper" if="{ currentOrder }">
			...
			<div class="order-detail-list">
				<li>
					<span style="width: 10%">项</span>
					<span style="width: 38%">品名</span>
				...
				<ul>
					<li each="{ item in orderDetail.list }">
						...
						<span style="width: 13%">{ item.quantity || ' '}</span>
					 ...
					 <span style="width: 10%">{ orderDetail.list.indexOf(item) + 1}</span>
					 <span style="width: 38%">{ item.goodsName || '无码商品' }</span>
					 <span style="width: 15%">{ item.weight || ' '}</span>
					 <span style="width: 15%">{ item.price || ' ' }</span>
					 <span style="width: 15%">{ item.amount || ' ' }</span>
					</li>
				</ul>
			</div>
			...
			<div class="order-summary">
				<!-- <div>
					...
					<span class="pull-right">折扣：{ (orderDetail.discountPct || 100) + '%' }</span>
				</div>
				<div>
					<span>总数：{ countNumber(orderDetail.list) }</span>
					<span class="pull-right">总金额：<b>￥{ orderDetail.goodsAmount }</b></span>
				</div> -->
				<div class="amount">
	...
	<script>
		...
		self.countNumber = function(arr) {
			...
			arr.forEach(function(item) {
				if (item.weight) {
					number = (number*1000 + item.weight*1000)/1000;
				}
			});
			return number;
		}
