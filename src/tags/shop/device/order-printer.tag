...
<!-- 				</div> -->
			...
			<div>
				...
				<!-- <p>确认订单时是否打印小票 <span class="switch { close: !orderPrint}" onclick="{ switchOrderPrint }"></span></p>
				<p>是否打印店铺二维码 <span class="switch { close: !qrcode}" onclick="{ switchQrcode }"></span></p> -->
			</div>
		</div>
	</div>
		...
		self.getOrderPrintState = function(){
			httpGet({
				...
				complete: function(data) {
					self.orderPrint = data.printOrderState;
					self.update();
				}
			});
		}
