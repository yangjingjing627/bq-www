<casher-index>
	<div class="content-wrap">
		<div class="cart">
			...
			<div class="actions" if="{ !moreButton }">
			...
			<div class="actions" if="{ moreButton }">
				<span class="back-action" onclick={ backAction }></span>
				<span onclick={ saveBill }>挂单</span>
				<span onclick={ openBill }>取单</span>
				<span onclick={ openRefund }>退货</span>
			</div>
		</div>
		<div class="borad">
			<div id="casher-view">
			...
			<!-- <div class="vip-btn">
				<a onclick={ vipModify } if={ !vipLogin }>
					...
					<span class="text line vip-nologin">网店会员登录</span>
				</a>
				 <a onclick={ vipModify } if={ vipLogin }>
					<span class="vip-icon vip-login"></span>
					<span class="text vip-login">会员   <i>{ vipphone }</i></span>
					<span class="modify text line">修改</span>
				</a>
			</div> -->
			<div class="balance-btn">
				<a onclick={ goCheck } class={ disable: cartList.goodsAmount == 0 }>收银
...
<!-- 				</form> -->
	...
	<script>
		...
		self.goCheck = function () {
			if (self.cartList.goodsAmount == 0) {
				return;
			}
			if (self.cartList.list.length > 0) {
...
//  			e.preventDefault();
		...
		self.on('mount', function () {
			...
			flux.bind.call(self, {
				...
				store: store.cart,
				success: function () {
					if (self.cartList && self.cartList.goodsAmount) {
						self.cartList.goodsAmount = self.cartList.goodsAmount.toFixed(1);
					}
					self.update();
				}
			});
